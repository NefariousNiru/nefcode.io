...
function normalizeFileNameFromUrl(url: string): string {
    ...
    return decoded.toLowerCase().endsWith(".csv") ? decoded.slice(0, -4) : decoded;
}
...
function parseCompaniesShape(obj: UnknownRecord): ManifestCompany[] | null {
    ...
    for (const c of companiesVal) {
        ...
        for (const f of filesVal) {
            ...
            files.push({name: nameRaw?.trim() || normalizeFileNameFromUrl(url), url});
        }
...
function buildCompaniesFromEntries(entries: unknown[]): ManifestCompany[] | null {
...
function parseEntriesShape(obj: UnknownRecord): ManifestCompany[] | null {
    const entries =
        obj.entries ?? obj.files ?? obj.items;
...
export async function fetchManifest(signal?: AbortSignal): Promise<Manifest> {
    ...
    if (!companies) {
        throw new Error("Unrecognized manifest.json shape. Expected top-level array or companies[]/entries[].");
    }
