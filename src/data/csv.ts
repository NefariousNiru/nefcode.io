 ...
 */
export async function fetchCsvText(urlRelative: string, signal?: AbortSignal): Promise<string> {
    const base = import.meta.env.BASE_URL;
    const res = await fetch(`${base}${urlRelative}`, {signal});
    if (!res.ok) {
        throw new Error(`Failed to fetch CSV: ${urlRelative} (${res.status} ${res.statusText})`);
    }
    return res.text();
}
 ...
 */
export async function loadCsv(urlRelative: string, signal?: AbortSignal): Promise<CsvLoadResult> {
